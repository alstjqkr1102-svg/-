...
def calculate_path_energy(phi_nodes):
    ...
    # 각 점에서의 위도 각도(phi)를 이용한 수직항력 N = m * g * cos(phi)
    phi_mid = (phi_arr[:-1] + phi_arr[1:]) / 2.0
    N_arr = m * g * np.cos(phi_mid)
    
    # 구간별 수직항력 가중평균 (평균 수직항력 N_avg)
...
with c3:
    st.metric(label="🔥 마찰 손실 에너지 (μ · N_avg · L)", value=f"{res['e_fric']:.4f} J")
